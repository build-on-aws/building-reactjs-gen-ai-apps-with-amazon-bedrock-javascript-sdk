...
const buildOverrideDir = async (cwd, destDirPath) => {
    ...
    if ((0, index_1.skipHooks)()) {
        throw new index_1.AmplifyError('ScriptingFeaturesDisabledError', {
    ...
    const packageManager = await (0, index_1.getPackageManager)(cwd);
...
const setSettingsJsonAwscloudformationFlagFalse = () => {
    if (index_1.stateManager.getLocalEnvInfo().defaultEditor !== 'vscode') {
        return;
    }
    ...
    try {
        const settings = jsonUtilities_1.JSONUtilities.readJson(workspaceSettingsPath);
        jsonUtilities_1.JSONUtilities.writeJson(workspaceSettingsPath, (0, lodash_1.merge)(settings, exclusionRules));
    }
    catch (error) {
    }
};
