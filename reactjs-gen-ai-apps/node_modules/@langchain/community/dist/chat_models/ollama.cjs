...
const ollama_js_1 = require("../utils/ollama.cjs");
/**
 ...
 * ```
 */
class ChatOllama extends chat_models_1.SimpleChatModel {
    static lc_name() {
        return "ChatOllama";
    }
    constructor(fields) {
    ...
    async *_streamResponseChunksLegacy(input, options, runManager) {
        const stream = (0, ollama_js_1.createOllamaGenerateStream)(this.baseUrl, {
        ...
        for await (const chunk of stream) {
            if (!chunk.done) {
                yield new outputs_1.ChatGenerationChunk({
                    text: chunk.response,
                    message: new messages_1.AIMessageChunk({ content: chunk.response }),
                });
    ...
    async *_streamResponseChunks(input, options, runManager) {
        try {
            const stream = await this.caller.call(async () => (0, ollama_js_1.createOllamaChatStream)(this.baseUrl, {
                ...this.invocationParams(options),
            ...
            for await (const chunk of stream) {
                if (!chunk.done) {
                    yield new outputs_1.ChatGenerationChunk({
                        ...
                        message: new messages_1.AIMessageChunk({ content: chunk.message.content }),
                    });
