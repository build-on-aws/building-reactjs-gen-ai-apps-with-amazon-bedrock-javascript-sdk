...
import { GraphQLAuthMode, DocumentType } from '@aws-amplify/core/internals/utils';
 ...
 */
type WithListsFixed<T> = T extends PagedList<infer IT, infer NAME> ? PagedList<Exclude<IT, null | undefined>, NAME> : T extends {} ? {
 ...
 */
type FixedQueryResult<T> = Exclude<T[keyof T], null | undefined> extends PagedList<any, any> ? {
 ...
 * ```
 */
export type GraphqlSubscriptionResult<T> = Observable<GraphqlSubscriptionMessage<T>>;
/**
...
export type AWSAppSyncRealTimeProvider = {
    subscribe(options?: AWSAppSyncRealTimeProviderOptions): Observable<Record<string, unknown>>;
};
 ...
 */
export interface GraphQLOptionsV6<FALLBACK_TYPES = unknown, TYPED_GQL_STRING extends string = string> {
...
export declare const __amplify: unique symbol;
export declare const __authMode: unique symbol;
export declare const __authToken: unique symbol;
export declare const __headers: unique symbol;
export type ClientWithModels = V6Client<Record<string, any>> | V6ClientSSRRequest<Record<string, any>> | V6ClientSSRCookies<Record<string, any>>;
...
export type V6ClientSSRRequest<T extends Record<any, any> = never> = ExcludeNeverFields<{
    [__amplify]: AmplifyClassV6;
    [__authMode]?: GraphQLAuthMode;
    [__authToken]?: string;
    [__headers]?: CustomHeaders;
...
export type V6ClientSSRCookies<T extends Record<any, any> = never> = ExcludeNeverFields<{
    [__amplify]: AmplifyClassV6;
    [__authMode]?: GraphQLAuthMode;
    [__authToken]?: string;
    [__headers]?: CustomHeaders;
...
export type GraphQLMethod = <FALLBACK_TYPES = unknown, TYPED_GQL_STRING extends string = string>(options: GraphQLOptionsV6<FALLBACK_TYPES, TYPED_GQL_STRING>, additionalHeaders?: CustomHeaders | undefined) => GraphQLResponseV6<FALLBACK_TYPES, TYPED_GQL_STRING>;
export type GraphQLMethodSSR = <FALLBACK_TYPES = unknown, TYPED_GQL_STRING extends string = string>(contextSpec: AmplifyServer.ContextSpec, options: GraphQLOptionsV6<FALLBACK_TYPES, TYPED_GQL_STRING>, additionalHeaders?: CustomHeaders | undefined) => GraphQLResponseV6<FALLBACK_TYPES, TYPED_GQL_STRING>;
/**
 * @private
 *
...
export type ServerClientGenerationParams = {
    amplify: null | ((fn: (amplify: AmplifyClassV6) => Promise<any>) => Promise<any>);
